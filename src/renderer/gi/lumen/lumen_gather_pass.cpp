...
void LumenGatherPass::bindResources(const LumenResources& res,const RenderTargets& rt,VkBuffer frameUbo,bool useFiltered){ auto& vkD=static_cast<rhi::VkRHIDevice&>(*m_rhiDevice);
    const rhi::RHITextureView* av=useFiltered?res.filteredAtlasRhiView():res.probeAtlasRhiView();
    m_set->write({{0,rhi::DescriptorType::UniformBuffer,nullptr,rhi::VkRHIBuffer::createNonOwning(vkD,frameUbo,VK_WHOLE_SIZE).get()},{1,rhi::DescriptorType::SampledImage,av},{2,rhi::DescriptorType::Sampler,nullptr,nullptr,0,0,m_pointClamp.get()},{3,rhi::DescriptorType::SampledImage,rt.rhiGAlbedoMetalView()},{4,rhi::DescriptorType::SampledImage,rt.rhiGNormalRoughView()},{5,rhi::DescriptorType::SampledImage,rt.rhiDepthView()},{6,rhi::DescriptorType::StorageImage,rt.rhiLumenGIView()}});
}
