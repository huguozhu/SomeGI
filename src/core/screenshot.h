...
//            oneShotSubmitRHI(..., [&](rhiCmd) { m_screenshot.recordCopy(rhiCmd, ...); });
...
public:
    ...
    // 通过 RHI 录制 copyTextureToBuffer（srcImage → staging）
    // 要求 srcImage 必须已在 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
    void recordCopy(rhi::RHICommandBuffer& rhiCmd, VkImage srcImage, VkExtent2D extent);
